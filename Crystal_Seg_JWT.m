...
outDir = 'E:\Movies\20210418_Ana_-N_-Buffer_+CaCO3\'
outName = 'mask';

%Create the linker object
LAP = LAPLinker;

vid = VideoWriter('E:\Movies\20210418_Ana_-N_-Buffer_+CaCO3\output.avi');
vid.FrameRate = 10;
open(vid);
...
%     pause
    ...
    CFP = imdilate(CFP, strel ('disk',10));
    ...
    Crystals = imopen(Crystals, strel ('disk',6));
    Crystals = imclose(Crystals, strel ('disk',20));
%     Crystals = imdilate(Crystals, strel ('disk',20));
%     Crystals = imdilate(Crystals, strel ('disk',10));
    ...
    % Tracking code
    LAP = assignToTrack(LAP, iFrame, rpCrystals);

    Iout = uint16(Crystals * 65535);
    %Make a movie
    for iAT = 1:numel(LAP.activeTrackIDs)

        %Get track data
        currTrack = getTrack(LAP, iAT);

        Iout = insertText(Iout, currTrack.Centroid(end, :), iAT, ...
            'FontSize', 32);
...
CFPperim = bwperim(Crystals);
imshowpair (CFPend, CFPperim)
pause (0.5);

Crystals

if iFrame == 1
    imwrite(Crystals, fullfile(outDir,[outName,'FromTif','.tif']), 'compression', 'none');
else
    imwrite(Crystals, fullfile(outDir,[outName,'FromTif','.tif']), 'writeMode', 'append', 'compression', 'none');
end
...
Crystals = imclose(Crystals, strel ('disk',20));
Crystals = imdilate(Crystals, strel ('disk',30));
